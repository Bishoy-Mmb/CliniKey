...
**Created**: 2026-05-21  
**Status**: Draft  
**Input**: User description: "Implement the tenant provisioning lifecycle for CliniKey — the automated process of creating, configuring, and managing clinic tenants. This phase bridges the gap between tenant identification (already in JWT claims) and tenant isolation (schema-per-tenant). It covers clinic onboarding, PostgreSQL schema creation, migration application to new schemas, schema switching in DbContext, and tenant lifecycle management (activation, deactivation, suspension). This is the foundational infrastructure that enforces the multi-tenant data isolation promised in Phase 001."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Onboard a New Clinic (Priority: P1)

A platform operator registers a new clinic on the CliniKey platform. They provide the clinic's name, phone number, and address. The system creates the clinic record, generates a unique schema name, provisions a dedicated PostgreSQL schema, applies all required table migrations to the new schema, and marks the clinic as active and ready for use.
...
**Independent Test**: Can be fully tested by calling the clinic onboarding endpoint, verifying the clinic record is created in the shared schema, and confirming that the dedicated PostgreSQL schema exists with all expected tables.

**Acceptance Scenarios**:

1. **Given** a platform operator provides clinic name "Cairo Dental Center", phone "01112345678", and address "15 Tahrir St, Cairo", **When** the onboarding request is submitted, **Then** the system creates a clinic record with a unique ID, generates a schema name (e.g., `tenant_<short_id>`), creates the PostgreSQL schema, applies all table migrations, and returns the clinic ID.
...
3. **Given** the database is temporarily unreachable during schema creation, **When** the onboarding request is processed, **Then** the system rolls back any partial state (no orphaned clinic record without a schema) and returns a clear error indicating the operation failed.
...
2. **Given** a user with a valid JWT containing `tenant_id: <clinic-a-id>`, **When** they make any API request, **Then** the system sets the PostgreSQL `search_path` to the clinic's schema before executing any database operation.
...
Certain data — specifically clinic records, dentist profiles, and the clinic-dentist association — is shared across tenants and must remain accessible regardless of which tenant schema is active. This data lives in a dedicated shared schema, queried independently of tenant schema switching.
...
### User Story 4 - Deactivate and Reactivate a Clinic (Priority: P2)

A platform operator can deactivate a clinic, preventing all users of that clinic from accessing the system. The clinic's data is preserved (not deleted), and the clinic can be reactivated later, restoring full access.
...
**Independent Test**: Can be tested by deactivating a clinic, attempting to access it as a user of that clinic (expecting rejection), then reactivating and verifying access is restored.

**Acceptance Scenarios**:

1. **Given** a platform operator deactivates Clinic A, **When** a user belonging to Clinic A attempts to make any API request, **Then** the system returns 403 Forbidden with an error indicating the clinic is inactive.
2. **Given** Clinic A is deactivated, **When** the platform operator queries the list of clinics, **Then** Clinic A appears with status `Inactive`, and its schema and data remain intact.
3. **Given** Clinic A was deactivated, **When** the platform operator reactivates it, **Then** users of Clinic A can once again access the system and all their data is intact.
4. **Given** Clinic A is deactivated, **When** a platform operator queries Clinic A's configuration, **Then** the system returns the clinic details including a deactivation timestamp and the operator who deactivated it.

---

### User Story 5 - Complete Clinic Entity with Contact Details (Priority: P2)

The existing Clinic domain entity is extended with the missing contact details (phone number, address) that were planned in Phase 001 but not yet implemented. These fields are required for clinic onboarding and for displaying clinic information.
...
- What happens to a deactivated clinic's scheduled appointments and pending invoices? → Data remains as-is; no cascading cancellations. When reactivated, the clinic resumes with all data intact. Notifications about upcoming appointments for deactivated clinics are suppressed.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST create a dedicated PostgreSQL schema for each new clinic during onboarding.
...
- **FR-003**: System MUST generate unique, deterministic schema names derived from the clinic's ID (e.g., `tenant_<short_id>`). Schema names MUST be valid PostgreSQL identifiers.
- **FR-004**: System MUST enforce atomicity during onboarding: if schema creation or migration fails, the clinic record MUST be rolled back — no orphaned records without a corresponding schema.
...
- **FR-006**: System MUST store cross-tenant entities (Clinic, Dentist, ClinicDentist) in a shared schema accessible regardless of the active tenant schema.
- **FR-007**: System MUST prevent any data access for users belonging to a deactivated clinic, returning an appropriate error.
- **FR-008**: System MUST support clinic deactivation and reactivation by a platform operator without data loss.
...
- **FR-011**: System MUST support querying the list of all clinics with their provisioning status (active, inactive, schema health).
...
- **Clinic (Extended)**: The tenant boundary entity, extended with contact details. Key attributes: name, phone (unique, validated), address, schema name (unique, auto-generated), is active, provisioning status, created date, deactivated date. Stored in the shared schema.
...
- **SC-004**: A deactivated clinic's users receive an error within 1 second of attempting any operation, and data is fully intact upon reactivation.
...
- **SC-006**: Onboarding failure (e.g., simulated migration failure) results in zero orphaned state — no clinic record without a schema, no schema without a clinic record.
...
- The `shared` schema holds Clinic, Dentist, and ClinicDentist tables. All other operational data lives in tenant-specific schemas.
...
- Subscription/billing for clinic tenants (SaaS billing) is out of scope. Deactivation is an operator action, not an automated billing event.
