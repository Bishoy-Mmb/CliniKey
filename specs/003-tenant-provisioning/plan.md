...
Implement the tenant provisioning lifecycle for CliniKey. This phase turns clinic onboarding into an atomic operation that creates a shared clinic registry record, generates an immutable PostgreSQL tenant schema, applies operational migrations to that schema, and makes tenant-aware requests use the resolved schema. It also moves cross-tenant data (`Clinic`, `Dentist`, `ClinicDentist`) into an explicit `shared` schema, keeps auth data in `public`, and adds clinic activation/deactivation plus schema health checks.
...
|------|--------|----------|
| Clean Architecture dependency flow | Pass | Domain models clinic lifecycle; Application defines provisioning abstractions/features; Infrastructure owns PostgreSQL schema/migration implementation; API stays thin |
| CQRS via MediatR | Pass | Tenant lifecycle commands and registry queries are planned as Application vertical slices |
| Result<T> expected failures | Pass | Duplicate phone, inactive clinic, missing schema, unhealthy schema, and provisioning failures are explicit errors |
...
| TimeProvider usage | Pass | Clinic lifecycle timestamps and tests use injected TimeProvider/FakeTimeProvider |
...
|-- CliniKey.Domain/
|   |-- Entities/
|   |   |-- Clinic.cs                 # extend phone, address, status, lifecycle timestamps
...
|   |-- Events/
|   |   |-- ClinicProvisionedEvent.cs
|   |   |-- ClinicActivatedEvent.cs
|   |   `-- ClinicDeactivatedEvent.cs
|   `-- Errors/
|       `-- TenantErrors.cs
...
**Structure Decision**: Tenant lifecycle use cases live under `Application/Features/Tenants`. Infrastructure owns PostgreSQL-specific schema creation, migration, health checks, and connection search-path setup. Shared registry data can either use a focused `SharedDbContext` or explicit schema mappings in `AppDbContext`; implementation should choose the least disruptive option while keeping tenant operational mappings separate from shared mappings.
...
|----------|-----|----------------------|
| Explicit `shared` schema for Clinic/Dentist/ClinicDentist | Cross-tenant data must remain readable regardless of active tenant schema | Leaving these tables in whatever schema search path selects risks accidental tenant-local copies |
